...
def train_one_epoch(dataloader, model, loss_fn, optimizer, t):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        # print("t*size + current :", t*size + current, "t:", t, "size:", size, "batch:", batch, "current:", current)
        wandb.log({"train_loss": loss, "num_train_examples": total_steps_this_epoch})
        if batch % 10 == 0:
...
def main(args):
    ...
    if args.use_wandb:
        ...
        wandb.init(entity="stablegradients", project="hw0_img_classifier_shrinivr", name="neural-the-narwhal", config=hyperparameters)
    else:
    ...
    for t in range(args.n_epochs):
        ...
        test_correct, test_loss = evaluate(test_dataloader, "Test", model, loss_fn)
        wandb.log({"train_accuracy": train_correct, "train_loss": train_loss, 
                   "test_accuracy": test_correct, "test_loss": test_loss, "epoch": t+1})
