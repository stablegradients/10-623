...
def evaluate(dataloader, dataname, model, loss_fn, is_last_epoch=False, tag=''):
    ...
    with torch.no_grad():
        for X, y in dataloader:
            ...
            step_counter += 1
            print("step_counter:", step_counter, "is_last_epoch:", is_last_epoch)
            if is_last_epoch and step_counter == 1:
                print("logging images")
                for i in range(len(X)):
                    # caption is "<predicted label> / <true label>"
                    caption = tag + "_" + str(pred[i].argmax(0).item()) +  " / " + str(y[i].item())
                    print("caption:", caption)
                    wandb.Image(denormalize(X[i]), caption=caption, mode="RGB")
    avg_loss /= size
    correct /= size
...
def main(args):
    ...
    if args.use_wandb:
        ...
        wandb.init(entity="stablegradients", project="hw0_img_classifier_shrinivr", name="neural-the-narwhal-image-logging", config=hyperparameters)
    else:
